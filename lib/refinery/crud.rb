...
module Refinery
  module Crud
    ...
    def self.search_all_from_model(model_name, options)
      %(
        # First find normal results.
        find_all_#{options[:plural_name]}(#{options[:search_conditions].inspect})

        # Now get weighted results by running the query against the results already found.
        @#{options[:plural_name]} = @#{options[:plural_name]}.with_query(params[:search])
      )
    end
    ...
    module ClassMethods
      ...
      def crudify(model_name, options = {})
        ...
        search_all_from_model = ::Refinery::Crud.search_all_from_model(model_name, options)
